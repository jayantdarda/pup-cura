...
(function () {
  ...
  /* ---------------------------------------------------------------
     6. TOPIC TITLE TEXT-RISE — the one signature motion.
        Each .topic__intro gets .is-visible when it enters view,
        which fires the masked text-rise on its inner span.
        Mirrors the old section-opener reveal but scoped to .topic.
  --------------------------------------------------------------- */
  (function topicTitleReveal() {
    const intros = document.querySelectorAll(".topic__intro");
    if (!intros.length) return;

    if (!("IntersectionObserver" in window)) {
      intros.forEach((el) => el.classList.add("is-visible"));
      return;
    }
    ...
    intros.forEach((el) => obs.observe(el));
  })();
  ...
  (function mediaSlotLoader() {
    ...
    slots.forEach((slot) => {
      ...
      img.addEventListener("error", () => {
        // Source failed — leave the placeholder visible.
        slot.dataset.src = "";
      }, { once: true });
      img.src = src;
    });
  ...
  (function contactForm() {
    ...
    const SUCCESS_MSG = "Thanks — we'll reply within 48 hours.";

    form.addEventListener("submit", (e) => {
      e.preventDefault();
      if (!form.checkValidity()) {
        // Let the browser surface native validation UI.
        form.reportValidity();
        return;
      }
      if (status) {
